...
export default function ProjectDetailPage({ project, detail }: { project: ProjectRecord; detail?: ProjectDetailContent }) {
  ...
  return (
    <main id="main-content" className={`project-detail origin-${project.origin.toLowerCase().replaceAll(" ", "-")}`}>
      ...
      {project.githubLinks?.length ? <section className="project-section project-repository-section"><div className="project-detail-inner project-two-col"><div><p className="eyebrow">Code / repository</p><h2>Read the implementation.</h2><p className="project-repository-note">Public code links are included where the matching project repository is available.</p></div><div className="project-repository-links">{project.githubLinks.map((link) => <a key={link.url} href={link.url} target="_blank" rel="noreferrer"><span>{link.label}</span><strong>↗</strong></a>)}</div></div></section> : null}
